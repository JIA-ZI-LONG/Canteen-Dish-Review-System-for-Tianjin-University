        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="窗口名称" prop="name">
              <el-input v-model="stallForm.name" placeholder="请输入窗口名" prefix-icon="el-icon-shop"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    fetchSupportData() {
      // 获取食堂列表用于下拉�?(获取所有，所以设置一个较大的size)
      request({ url: '/admin/canteens', method: 'get', params: { current: 1, size: 1000 } }).then(res => { this.canteenList = res.records || [] })
      // 获取窗口类型列表用于下拉�?
      request({ url: '/admin/stall-types', method: 'get' }).then(res => { this.stallTypeList = res || [] })
    },
    ...
    updateData() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          request({
            url: `/admin/stalls/${this.stallForm.id}`,
            method: 'put',
            data: this.submitModel
          }).then(() => {
            this.dialogFormVisible = false
