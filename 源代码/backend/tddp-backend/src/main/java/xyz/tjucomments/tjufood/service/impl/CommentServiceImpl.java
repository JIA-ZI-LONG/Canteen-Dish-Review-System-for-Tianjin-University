...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements ICommentService {
    ...
    public Result addComment(Comment comment) {
        ...
        cacheRebuildExecutor.submit(() -> {
            try {
                incentiveService.processUserAction(currentUser.getId(), "ADD_COMMENT", "发表评论");
            } catch (Exception e) {
                log.error("处理发表评论积分奖励失败", e);
            }
        });
    ...
    @Override
    public Result queryCommentsByUser(Integer current, Integer size) {
