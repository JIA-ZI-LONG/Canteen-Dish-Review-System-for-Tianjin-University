...
public class CanteenTypeAdminController {
    ...
    public Result addCanteenType(@RequestBody CanteenType canteenType) {
        if (canteenType.getCreateTime() == null) {
            canteenType.setCreateTime(java.time.LocalDateTime.now());
        }
    ...
    @GetMapping
    public Result getCanteenTypes() {
        // 【修复】添加排序条件，避免SQL Server分页问题
        List<CanteenType> list = canteenTypeService.list(
            new QueryWrapper<CanteenType>().orderByAsc("id")
        );
