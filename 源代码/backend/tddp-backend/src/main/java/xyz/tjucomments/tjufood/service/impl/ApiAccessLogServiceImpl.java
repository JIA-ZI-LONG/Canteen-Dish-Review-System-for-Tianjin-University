...
public class ApiAccessLogServiceImpl extends ServiceImpl<ApiAccessLogMapper, ApiAccessLog> implements IApiAccessLogService {
    ...
    public Result getApiAccessLogs(Integer current, Integer size, String apiPath,
        ...
        if (apiPath != null && !apiPath.trim().isEmpty()) {
            queryWrapper.like(ApiAccessLog::getApiPath, apiPath);
        }

        if (status != null) {
            queryWrapper.eq(ApiAccessLog::getResponseStatus, status);
        }
        ...
        if (startTime != null && !startTime.trim().isEmpty()) {
            try {
                ...
                queryWrapper.ge(ApiAccessLog::getCreateTime, start);
            } catch (Exception e) {
                log.warn("API访问日志开始时间格式错误: {}", startTime);
            }
        }
        ...
        if (endTime != null && !endTime.trim().isEmpty()) {
            try {
                LocalDateTime end = LocalDateTime.parse(endTime + " 23:59:59",
                    DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
                queryWrapper.le(ApiAccessLog::getCreateTime, end);
            } catch (Exception e) {
                log.warn("API访问日志结束时间格式错误: {}", endTime);
            }
        }

        queryWrapper.orderByDesc(ApiAccessLog::getCreateTime);

        IPage<ApiAccessLog> logPage = page(page, queryWrapper);
    ...
    public Result getApiErrorStats(String startTime, String endTime) {
        // 构建基础查询条件
        LambdaQueryWrapper<ApiAccessLog> baseQuery = new LambdaQueryWrapper<>();
        ...
        if (startTime != null && !startTime.trim().isEmpty()) {
            try {
                LocalDateTime start = LocalDateTime.parse(startTime + " 00:00:00",
                    DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
                baseQuery.ge(ApiAccessLog::getCreateTime, start);
            } catch (Exception e) {
                log.warn("API错误统计开始时间格式错误: {}", startTime);
            }
        }
        ...
        if (endTime != null && !endTime.trim().isEmpty()) {
            try {
                LocalDateTime end = LocalDateTime.parse(endTime + " 23:59:59",
                    DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
                baseQuery.le(ApiAccessLog::getCreateTime, end);
            } catch (Exception e) {
                log.warn("API错误统计结束时间格式错误: {}", endTime);
            }
        }

        // 统计4xx错误
        LambdaQueryWrapper<ApiAccessLog> client4xxQuery = baseQuery.clone();
        client4xxQuery.ge(ApiAccessLog::getResponseStatus, 400)
                     .lt(ApiAccessLog::getResponseStatus, 500);
        long client4xx = count(client4xxQuery);

        // 统计5xx错误
        LambdaQueryWrapper<ApiAccessLog> server5xxQuery = baseQuery.clone();
        server5xxQuery.ge(ApiAccessLog::getResponseStatus, 500);
        long server5xx = count(server5xxQuery);

        // 统计超时错误（响应时间超过5秒）
        LambdaQueryWrapper<ApiAccessLog> timeoutQuery = baseQuery.clone();
        timeoutQuery.gt(ApiAccessLog::getResponseTime, 5000);
        long timeout = count(timeoutQuery);

        // 统计总请求数
        long totalRequests = count(baseQuery);

        Map<String, Object> errorStats = new HashMap<>();
        errorStats.put("client4xx", client4xx);
        errorStats.put("server5xx", server5xx);
        errorStats.put("timeout", timeout);
        errorStats.put("totalErrors", client4xx + server5xx);
        errorStats.put("totalRequests", totalRequests);
        errorStats.put("errorRate", totalRequests > 0 ?
            Math.round((double) (client4xx + server5xx) / totalRequests * 100 * 100.0) / 100.0 : 0);
    ...
    public Result getApiCallTrends(String startTime, String endTime, String granularity) {
        ...
        if (startTime != null && !startTime.trim().isEmpty()) {
            try {
                ...
                log.warn("API趋势统计开始时间格式错误: {}", startTime);
            }
        }
        ...
        if (endTime != null && !endTime.trim().isEmpty()) {
            try {
                ...
                log.warn("API趋势统计结束时间格式错误: {}", endTime);
            }
        }

        // 获取数据并按时间分组统计
        List<ApiAccessLog> logs = list(baseQuery.orderByAsc(ApiAccessLog::getCreateTime));

        // 简化实现：按小时统计
        Map<String, Long> hourlyStats = logs.stream()
            .collect(Collectors.groupingBy(
                log -> log.getCreateTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH")),
                Collectors.counting()
            ));

        trends.put("hourlyStats", hourlyStats);
        trends.put("totalCalls", logs.size());
    ...
    public Result getApiErrorRates(String startTime, String endTime) {
        // 复用错误统计的逻辑
        Result errorStatsResult = getApiErrorStats(startTime, endTime);
        Map<String, Object> errorStats = (Map<String, Object>) errorStatsResult.getData();

        Map<String, Object> errorRates = new HashMap<>();
        errorRates.put("client4xxRate", errorStats.get("client4xx"));
        errorRates.put("server5xxRate", errorStats.get("server5xx"));
        errorRates.put("timeoutRate", errorStats.get("timeout"));
        errorRates.put("overallErrorRate", errorStats.get("errorRate"));
