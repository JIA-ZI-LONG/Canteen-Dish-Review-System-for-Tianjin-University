...
@Tag(name = "B03. 内容管理 - 窗口管理", description = "对窗口信息进行增删改查及图片管理") // 【修改】更新描述
...
public class StallAdminController {
    ...
    // 【新增】定义文件类型常量
    private static final int FILE_TYPE_STALL = 2;

    @Log(module = "窗口管理", operation = "创建窗口")
    @Operation(summary = "创建窗口")
    @PostMapping
    public Result createStall(@RequestBody Stall stall) {
        // 验证必需字段
        ...
        return isSuccess ? Result.ok(stall.getId()) : Result.fail("创建窗口失败！");
    }
    ...
    @Log(module = "窗口管理", operation = "更新窗口")
    @Operation(summary = "更新窗口信息")
    @PutMapping("/{id}")
    public Result updateStall(@PathVariable Long id, @RequestBody Stall stall) {
        stall.setId(id); // 确保ID一致性
        if (id == null) {
            return Result.fail("更新失败，窗口ID不能为空");
        }

        // 验证窗口是否存在
        Stall existingStall = stallService.getById(id);
    ...
    @GetMapping("/{id}")
    public Result getStall(@Parameter(description = "窗口唯一标识ID", required = true) @PathVariable Long id) {
