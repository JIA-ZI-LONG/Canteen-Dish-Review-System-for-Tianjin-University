...
public class IncentiveRuleAdminController {
    ...
    @GetMapping
    public Result getIncentiveRules(
    ...
    @Operation(summary = "获取激励统计信息")
    @GetMapping("/statistics")
    public Result getIncentiveStatistics() {
        return incentiveService.getIncentiveStatistics();
    }

    @Operation(summary = "启用/禁用激励规则")
    @PatchMapping("/{id}/status")
    public Result toggleIncentiveRuleStatus(@PathVariable Long id, @RequestParam Integer status) {
        if (status == null || (status != 0 && status != 1)) {
            return Result.fail("状态值无效，只能是0（禁用）或1（启用）");
        }

        IncentiveRule rule = incentiveRuleService.getById(id);
        if (rule == null) {
            return Result.fail("激励规则不存在");
        }

        rule.setStatus(status);
        boolean isSuccess = incentiveRuleService.updateById(rule);
        return isSuccess ? Result.ok("状态更新成功") : Result.fail("状态更新失败");
    }
}