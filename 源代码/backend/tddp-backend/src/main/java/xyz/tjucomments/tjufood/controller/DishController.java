...
public class DishController {
    ...
    public Result searchDishes(
            ...
            @Parameter(description = "校区ID，用于筛选") @RequestParam(value = "campusId", required = false) Long campusId,
            @Parameter(description = "食堂ID，用于筛选") @RequestParam(value = "canteenId", required = false) Long canteenId,
            @Parameter(description = "排序字段 (name=按名字, price=按价格, liked=按点赞)", example = "liked")
            @RequestParam(value = "sortBy", defaultValue = "liked") String sortBy,
            @Parameter(description = "排序方向 (asc=升序, desc=降序)", example = "desc")
            @RequestParam(value = "sortOrder", defaultValue = "desc") String sortOrder) {
        return dishService.searchDishes(name, campusId, canteenId, current, size, sortBy, sortOrder);
    }
