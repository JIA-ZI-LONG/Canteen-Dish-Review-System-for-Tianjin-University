...
public class CampusAdminController {
    ...
    @Log(module = "校区管理", operation = "创建校区")
    @Operation(summary = "创建校区")
    @PostMapping
    public Result createCampus(@RequestBody Campus campus) {
        // 【高优先级修复】移除手动生成ID的逻辑，因为数据库是自增的
        // campus.setId(id) 等代码已删除
        boolean isSuccess = campusService.save(campus);
        return isSuccess ? Result.ok(campus.getId()) : Result.fail("创建校区失败！");
    }
    ...
    @Log(module = "校区管理", operation = "更新校区")
    @Operation(summary = "更新校区信息")
    @PutMapping("/{id}")
    public Result updateCampus(@PathVariable Long id, @RequestBody Campus campus) {
        campus.setId(id); // 确保ID一致性
        if (id == null) {
    ...
    @GetMapping
    public Result getCampuses(@RequestParam(value = "all", defaultValue = "true") boolean all) {
        // 校区数据通常不多，默认返回所有
