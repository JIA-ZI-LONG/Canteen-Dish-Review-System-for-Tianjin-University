...
public class CommentController {
    ...
    @Operation(summary = "查询当前登录用户的评论列表")
    @GetMapping("/user/me")
    @SecurityRequirement(name = "authorization")
    public Result listMyComments(@RequestParam(defaultValue = "1") Integer current,
                                 @RequestParam(defaultValue = "10") Integer size) {
        return commentService.queryCommentsByUser(current, size);
    }
}