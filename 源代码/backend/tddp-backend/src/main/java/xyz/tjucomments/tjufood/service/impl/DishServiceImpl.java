...
public class DishServiceImpl extends ServiceImpl<DishMapper, Dish> implements IDishService {
    ...
    public Result searchDishesByName(String name, Integer current, Integer size) {
        // 保持原有方法不变，调用新的重载方法
        // 默认按 liked 倒序
        return searchDishes(name, null, null, current, size, "liked", "desc");
    }

    @Override
    public Result searchDishesByName(String name, Integer current, Integer size, String sortBy, String sortOrder) {
        return searchDishes(name, null, null, current, size, sortBy, sortOrder);
    }

    @Override
    public Result searchDishes(String name, Long campusId, Long canteenId, Integer current, Integer size, String sortBy, String sortOrder) {
        // XML中的自定义多表查询返回DishDTO，支持自定义排序
        Page<DishDTO> page = new Page<>(current, size);
        List<DishDTO> dishes = baseMapper.searchDishesByNameWithSort(page, name, campusId, canteenId, sortBy, sortOrder);
